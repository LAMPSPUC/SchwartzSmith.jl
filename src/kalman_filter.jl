...
"""
    kalman_filter(ln_F::Matrix{Typ}, T::Matrix{Typ}, D::Matrix{Float64}, p::SSParams{Typ}, delta_t::Int) where Typ
...
function kalman_filter(ln_F::Matrix{Typ}, T::Matrix{Typ}, D::Matrix{Float64}, p::SSParams{Typ}, delta_t::Int) where Typ
    ...
    P_kf   = Array{Typ, 3}(undef, 2 + s, 2 + s, n+1)
    att_kf = Matrix{Typ}(undef, n, 2 + s)
    Ptt_kf = Array{Typ, 3}(undef, 2 + s, 2 + s, n)
...
"""
    kalman_filter(ln_F::Matrix{Typ}, T_V::Vector{Typ}, D::Matrix{Float64}, p::SSParams{Typ}, delta_t::Int) where Typ
